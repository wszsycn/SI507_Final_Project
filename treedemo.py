...
from program_shaozhi import process_name, directplot_tree
...
HEADERS = {'Authorization': 'Bearer {}'.format(API_KEY),
           'User-Agent': 'SI507 Final Project - Restaurant Searching System',
           'From': 'shaozhi@umich.edu'
}
...
@app.route('/plot_tree/<city_or_state>/<nm>/<choice>')
def Tree_chart(city_or_state, nm, choice):
    #print(f"nm: {nm}")  # Debug print
    if city_or_state == 'state':
        name = process_name(nm.replace('%20', ' '))
        #######
        #name = f"{id_pos}_{name}"
        if choice == 'directplot_tree':
            figure, chart_data = directplot_tree(name, city_or_state)
            ...
            return render_template('plot_tree.html', figure=Markup(serialized_figure), tree_data=tree_data, name=name)
        ...
        if choice == 'directplot_tree':
            figure, chart_data = directplot_tree(name, city_or_state, id_pos=id_pos)
            # Convert the fig object to JSON and pass it to your template
            serialized_figure = json.dumps(figure, cls=PlotlyJSONEncoder)
            #return render_template('plot_restaurant.html', chart_data=chart_data)
            tree_data = [
...
################ Classes would be used: City & Restaurant  ##################
...
#########################################
########### Data Processing #############
########### Create and write the according db #############
...
def flask_plot_directly(xvals, yvals, title, fig_type, chart_data=None):
    ''' this function directly plot the restaurant that is used for displaying in
        the html webpage
    ...
    if fig_type == 'pie':
        ...
        #fig.add_trace(go.Bar(x=xvals, y=yvals), row=1, col=1)
        # Add the customdata attribute to store the restaurant names for each category
...
def directplot_avgreview_each_category(name, target, id_pos=None):
    ''' a function that generate a barplot for the average number of reviews 
        of differnt restaurant types in the city or state specified 
    ...
    # query = '''SELECT r.Category, r.[Number of Review] FROM Restaurants as r
    #             JOIN Cities as c ON c.Id=r.City_id'''
    query = '''SELECT r.Category, r.[Number of Review], r.Name FROM Restaurants as r
            JOIN Cities as c ON c.Id=r.City_id'''
    ...
    #return flask_plot(xvals, yvals, title, 'bar')
        # Add a new line to get the restaurant names for each category
    category_restaurants = get_restaurants_by_category(results)

    title = 'Average Number of Reviews of Different Categories in {} {}'.format(name, target.capitalize())
    
    # Return the plot data and the category_restaurant mapping
    return flask_plot_directly(xvals, yvals, title, 'bar', chart_data=category_restaurants)
  
# Add a new function to get the restaurant names for each category
def get_restaurants_by_category(results):
...
def directplot_tree(name, target, id_pos=None):
    name = process_name(name)

    query = '''
        SELECT r.Category, r.[Number of Review], r.Name
...
def get_restaurants_by_category_with_image(results):
    category_restaurants = defaultdict(list)
    for result in results:
        category_restaurants[result[0]].append((result[2], result[3]))
    return category_restaurants
...
@app.route('/plot_restaurant/<city_or_state>/<nm>/<choice>')
def chart_data(city_or_state, nm, choice):
    #print(f"nm: {nm}")  # Debug print
    if city_or_state == 'state':
        name = process_name(nm.replace('%20', ' '))
        #######
        #name = f"{id_pos}_{name}"
        if choice == 'directplot_avgreview_each_category':
            figure, chart_data = directplot_avgreview_each_category(name, city_or_state)
            # Convert the fig object to JSON and pass it to your template
            serialized_figure = json.dumps(figure, cls=PlotlyJSONEncoder)
            #return render_template('plot_restaurant.html', chart_data=chart_data)
            ...
            return render_template('plot_restaurant.html', figure=Markup(serialized_figure), chart_data=chart_data, name=name)
            #return render_template('plot_restaurant.html', figure=Markup(serialized_figure), tree_data=tree_data, name=name)
        else:
            return "Invalid choice"
    else:
        id_city = nm.split('_')
        ...
        if choice == 'directplot_avgreview_each_category':
            figure, chart_data = directplot_avgreview_each_category(name, city_or_state, id_pos=id_pos)
            # Convert the fig object to JSON and pass it to your template
            serialized_figure = json.dumps(figure, cls=PlotlyJSONEncoder)
            #return render_template('plot_restaurant.html', chart_data=chart_data)
            tree_data = [
...
def data(city_or_state, nm, choice):
    ...
    html template that contains figure which is a plot.
    '''
    #chart_data = {}  # Initialize chart_data as an empty dictionary
    if city_or_state == 'state':
        name = process_name(nm.replace('%20', ' '))
        ...
        # elif choice == 'directplot_avgreview_each_category':
        #     figure, chart_data = directplot_avgreview_each_category(name, city_or_state)
        #     # Convert the fig object to JSON and pass it to your template
        #     serialized_figure = json.dumps(figure, cls=PlotlyJSONEncoder)
        #     return render_template('plot_restaurant.html', figure=Markup(serialized_figure), chart_data=chart_data)
    else:
        id_city = nm.split('_')
        id_pos = int(id_city[0])
        name = process_name(id_city[1].replace('%20', ' '))
        if choice == 'pieplot_restaurant_categories':
            ...
            figure = barplot_avgreview_each_category(name, city_or_state, id_pos=id_pos)
            #serialized_figure = json.dumps(figure, cls=PlotlyJSONEncoder)
            #return render_template('plot.html', figure=Markup(serialized_figure), chart_data=chart_data)
        elif choice == 'barplot_toprated_restaurant':
    ...
    #return render_template('plot.html', figure=Markup(figure), chart_data=chart_data)
    return render_template('plot.html', figure=Markup(figure))
